...
function install_apk() {
  ...
  if [ "$API" -ge 28 ]; then
    ...
    installMsg=$(cat "$apkPath" | pm install -r -S "$apkSize")
  else
    pm install -r "$apkPath" 1>/dev/null
  fi
  ...
  if [ "$?" -eq 0 ]; then
    ...
    apkSize=$(wc -c <"$apkPath")
    cat "$apkPath" | pm install --bypass-low-target-sdk-block -r -S "$apkSize"
  else
    mkdir -p ${cachePath}
    cp -p -a -R "$apkPath" ${cachePath}/temp.apk
    chmod 0777 ${cachePath}/temp.apk
    pm install -r ${cachePath}/temp.apk 1>&2
    if [ "$?" -eq 0 ]; then
      echo "${fileName}安装应用完成！"
      rm -rf ${defaultPath}
...
function install_apex() {
  apkSize=$(wc -c <"$apkPath")
  cat "$apkPath" | pm install -r -S "$apkSize" --apex 1>&2
  if [ "$?" -eq 0 ]; then
    echo "${fileName}安装应用完成！"
  else
    errorCode="$?"
