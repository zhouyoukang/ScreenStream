...
| 文件 | 关系 |
|------|------|
| `反向控制/输入路由/InputRoutes.kt` | 在 HttpServer 中通过 `installInputRoutes()` 挂载 |
| `反向控制/输入服务/InputService.kt` | 提供触控/键盘/导航等所有控制能力 |
| `反向控制/HTTP服务器/` | Input 独立 HTTP 服务（端口 8084） |
| `反向控制/宏系统/MacroEngine.kt` | 宏录制/回放引擎 |

### 7.2 直接依赖：`:common` 模块（基础设施）

| 文件 | 关系 |
|------|------|
| `基础设施/010-模块管理_ModuleManager/` | StreamingModule 基类/StreamingModuleService |
| `基础设施/020-依赖注入_DI/` | CommonKoinModule |
| `基础设施/030-通用工具_Utils/` | 扩展函数/ModuleSettings |
| `基础设施/040-日志系统_Logging/` | XLog 封装 |

### 7.3 上游宿主：`:app` 模块

`用户界面/` — 应用主入口，加载并展示 MJPEG 模块 UI

## 八、参考/伴侣文件（非编译依赖）

| 文件 | 性质 | 说明 |
|------|------|------|
| `构建部署/030-MJPEG构建_MjpegBuild` | 上游参考 | 原始 build.gradle.kts 参考版本 |
...
| 文件 | 问题 | 处置 |
|------|------|------|
| `配置管理/020-MJPEG配置_MjpegSettings/settings/` (3 files) | ❌ 与 `mjpeg/settings/` 100% 重复（已验证 byte-identical） | 应删除，避免维护漏改 |
| `配置管理/030-RTSP配置_RtspSettings/settings/` (2 files) | ❌ 与 RTSP 模块内 settings/ 重复 | 同上 |
| `配置管理/040-WebRTC配置_WebRtcSettings/settings/` (2 files) | ❌ 与 WebRTC 模块内 settings/ 重复 | 同上 |
