...
| `mjpeg/` | Web UI + MJPEG 输出、Ktor Server（对外入口之一） | `mjpeg/.../internal/HttpServer.kt` | `MjpegSettings`（模块内） | `docs/adr/ADR-20260210-input-http-entrypoints.md` | `skills/skill-ssv2-input-unify/` |
| `input/` | 远程控制：Accessibility + 输入注入 + API + AI Brain + 宏系统 | `input/.../InputRoutes.kt`、`input/.../InputService.kt`、`input/.../MacroEngine.kt` | `input/.../settings/InputSettings.kt`（端口/开关） | `docs/adr/ADR-20260210-input-http-entrypoints.md` | `skills/skill-ssv2-input-unify/` |
| `webrtc/` | WebRTC 输出链路 | `webrtc/src/...` | 模块 settings |  |  |
| `rtsp/` | RTSP 输出链路 | `rtsp/src/...` | 模块 settings |  |  |
| `Quest/VR 适配` | Quest/VR 浏览器兼容、裁切/交互策略（优先开关化） | 多分布于 `mjpeg/assets/index.html`、渲染链路 | URL 参数/设置项 |（必要时新建）| `skills/skill-ssv2-merge-plan/` |
...
- **MJPEG Server（主入口）**：
  ...
  - 已内置输入路由（v32+ 共 59 个功能）：
    - 基础：`/tap /swipe /pointer /key /text /home /back /recents /status /notifications`
    - v30 系统控制：`/volume/up /volume/down /lock /quicksettings`
    - v31 远程协助：`/wake /screenshot /power /splitscreen /brightness /longpress /doubletap /scroll /pinch /openapp /openurl /deviceinfo /apps /clipboard`
