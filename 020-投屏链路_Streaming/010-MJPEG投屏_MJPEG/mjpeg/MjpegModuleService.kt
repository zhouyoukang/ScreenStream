...
public class MjpegModuleService : StreamingModuleService() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        if (streamingModuleManager.isActive(MjpegStreamingModule.Id)) {
            when (mjpegEvent) {
                is MjpegEvent.Intentable.StartService -> {
                    try {
                        val notification = notificationHelper.createForegroundNotification(this, MjpegEvent.Intentable.StopStream("User action: Notification").toIntent(this))
                        ServiceCompat.startForeground(this, notificationIdForeground, notification, ServiceInfo.FOREGROUND_SERVICE_TYPE_MEDIA_PROJECTION)
                    } catch (e: Exception) {
                        XLog.w(getLog("onStartCommand", "startForeground failed: ${e.message}"))
                    }
                    mjpegStreamingModule.onServiceStart(this)
                }
