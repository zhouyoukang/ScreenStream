# 雷鸟V3 AR眼镜开发 · Agent操作指令

## 项目定位

RayNeo V3 (XRGF50) AI眼镜 → **万物中枢**。伏羲八卦统御11台设备，双主控制(用户+Agent)。

架构: 五感→道感知→三联道→**万物中枢(wan_wu_hub)**
...
- **万物中枢端口**: 8808 (Dashboard + API)

## 文件说明

| 文件 | 角色 | 修改风险 |
|------|------|----------|
| `wan_wu_hub.py` | **★万物中枢**（11设备×8适配器×双主控制） | 高 — 核心统御 |
| `wan_wu_server.py` | **★HTTP API + 八卦Dashboard** (:8808) | 中 — 依赖hub |
...
```
☰乾=眼镜V3  ☷坤=手机群  ☲离=电脑  ☳震=智能家居
☴巽=服务器   ☵坎=Quest3  ☶艮=手表  ☱兑=机器狗
```

双主控制:
- **用户**: 眼镜手势(tap/slide/long_press/nod/shake) → 意图映射 → 万物执行
- **Agent**: `python wan_wu_hub.py --agent "手机 home"` → 命令路由 → 设备执行

Dashboard: `http://localhost:8808` 或双击 `→万物中枢.cmd`
