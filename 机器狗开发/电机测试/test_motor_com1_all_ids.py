...
def test_all_motor_ids_com1():
    """使用COM1端口测试所有电机ID (0-12)"""
    port = "COM1"
    baudrate = 115200  # COM1最高支持的波特率
    duration_per_id = 3  # 每个ID测试3秒
    
    print(f"使用COM1端口测试所有电机ID (0-12)")
    print(f"波特率: {baudrate}")
    print(f"每个ID测试时长: {duration_per_id} 秒")
    print("=" * 40)
    ...
    try:
        ...
        for motor_id in range(13):  # 0-12
            ...
            # 生成正弦波命令
            while time.time() - start_time < duration_per_id:
                # 计算当前时间点的正弦值
