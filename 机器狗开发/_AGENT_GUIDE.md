...
**Phase 19 伏羲八卦·全量解压逆向·资源整合 (2026-03-09):**
115. **☲离·全景扫描** — 发现9个未解压GitHub zip(Genesis/IsaacLab/IsaacSim/CoppeliaSim/gazebo-models/gz-sim/mujoco-source/sapiens/webots, 合计~2GB)
116. **☳震·批量解压** — 9/9 zip全部解压到refs/目录, 总计30个参考项目, 38738文件, 4.7GB
117. **☲离·深度逆向** — 伏羲八卦逆向分析30个refs: 11个Go1模型文件 + 55个预训练策略(7 Go1专用) + 26个Go1目录 + 317个可复用Python文件
118. **🔴 sapiens嵌套目录Bug** — sapiens.zip解压后包含mujoco-main(重复)+sapiens-main两层嵌套 → 删除重复mujoco-main, 提升sapiens-main内容到顶层
119. **☳震·go1_resources.py** — 资源整合引擎(~310行): 全景报告+Go1模型扫描+预训练策略索引+可复用代码扫描+八卦审计, 30个项目元数据(Tier1-4分层+Go1相关标记+复用说明)
120. **☴巽·最小成本评估** — 按价值分层: T1核心Go1直用(11项)→T2生态高价值(8项)→T3参考(7项)→T4低价值(4项); 识别待集成高价值: walk-these-ways(RL部署)+IsaacLab(actuator net)+unitree-guide(MPC/WBC)
121. **☷坤·.gitignore更新** — 添加9个zip文件+9个大型非核心refs目录(Genesis/IsaacLab/IsaacSim/coppeliaSimLib/gazebo-models/gz-sim/mujoco-source/sapiens/webots)到gitignore, 防止git膨胀
122. **☰乾·go1.py v3.1** — 统一CLI新增`resources`模块命令, 版本升至3.1
123. **☱兑·refs/README.md更新** — 新增9个解压项目文档(IsaacLab Go1详情+MuJoCo源码可复用部分)+资源整合引擎使用说明
124. **☵坎·全链路验证** — 14/14 py_compile PASS (8核心+6 gooddawg), go1_resources.py全景报告输出正确

**Phase 20 NVIDIA仿真生态深度整合·浏览器五感验证 (2026-03-09):**
125. **☲离·深度逆向IsaacLab** — 提取UNITREE_GO1_CFG完整配置: ActuatorNetMLP(effort=23.7Nm, vel=30rad/s, saturation=23.7Nm, pos_scale=-1.0), 初始关节角(hip±0.1, thigh F=0.8/R=1.0, calf=-1.5), 刚体属性(solver iter=4/0)
126. **☲离·深度逆向walk-these-ways** — 提取Go1 RL Sim2Real配置: Kp=20/Kd=0.5(实机低增益), action_scale=0.25, hip_scale_reduction=0.5, 域随机化(mass±[-1,3]kg, friction[0.05,4.5], motor_strength[0.9,1.1]), 确认预训练权重ac_weights_last.pt(12MB)+actuator_net unitree_go1.pt(16KB)均存在
127. **☲离·深度逆向cpg-go1-sim** — 分析847行CPG8Neuron: 8神经元Stein振荡器+Runge-Kutta 4阶积分, 5种步态(walk/trot/pace/bound/pronk), 4种切换策略(Switch/PowerPair/Wait&Switch/Wait&PowerPair), 耦合矩阵lambda_1/lambda_2
128. **☲离·深度逆向unitree-guide** — 提取go1_const.h官方SDK关节限位: hip[-1.047,1.047]±60°, thigh[-0.663,2.966]-38°~170°, calf[-2.721,-0.837]-156°~-48°
129. **🔴 关节限位差异发现** — MuJoCo menagerie限位 vs SDK限位存在显著差异: hip Δ0.184rad, thigh_max Δ1.535rad(MuJoCo允许257.9°远超SDK的170°) → 仿真用MuJoCo限位, 实机部署用SDK限位
130. **🔴 PD增益分歧发现** — walk-these-ways实机Kp=20/Kd=0.5 vs MuJoCo仿真Kp=100/Kd=2, 差5倍 → 实机模式和仿真模式应使用不同增益
131. **☳震·go1_nvidia_integration.py** — 创建NVIDIA仿真生态整合模块(~280行): 8维数据(IsaacLab actuator net + WTW RL Sim2Real + CPG Stein + unitree-guide SDK + MuJoCo tools + GenLoco + free-dog-sdk + unitree-ros), 统一Go1参数交叉验证, 关节限位对比, PD增益对比
132. **☳震·go1_sim.py修复** — 添加joint_limits_sdk_rad(go1_const.h官方限位)作为双模式安全参数
133. **☵坎·浏览器五感验证** — 启动Brain API :8085 + Dashboard浏览器实测: 🟢已连接✅ | 3D机器人渲染✅ | 站立命令✅ | 小跑步态✅(关节运动+IMU Y:-52.2°实时更新) | WASD提示✅ | 57个按钮全渲染✅ | 地形/速度选择器✅ | 足端力球体✅
134. **☳震·go1.py v3.1** — 统一CLI新增nvidia模块(go1_nvidia_integration.py), 共8个模块(sim/brain/rl/control/test/resources/nvidia)
135. **☶艮·问题清单** — STL加载警告(calf.stl fetch失败, 非阻塞, 3D模型用几何体替代已正常工作); Three.js r150+废弃警告(不影响功能)

**Phase 21 Isaac全量整合·go1_sim+go1_rl深度升级 (2026-03-09):**
136. **☲离·深度提取IsaacLab velocity_env_cfg** — 330行完整RL环境配置: 场景(terrain+robot+sensors) + MDP(commands+actions+observations+events+rewards+terminations+curriculum), 提取全部Go1-specific覆盖(rough_env_cfg 86行 + flat_env_cfg 44行)
137. **☲离·提取IsaacLab rewards.py** — 5个奖励函数实现: feet_air_time(L2核+零命令过滤) + feet_slide(接触力×速度惩罚) + track_lin_vel_xy_yaw_frame_exp(重力对齐速度跟踪) + track_ang_vel_z_world_exp + stand_still_joint_deviation_l1
138. **☲离·提取IsaacLab PPO配置** — rough: actor[512,256,128] lr=1e-3 gamma=0.99 lam=0.95 clip=0.2 ent=0.01 max_iter=1500; flat: actor[128,128,128] max_iter=300
139. **☲离·提取IsaacLab 域随机化** — friction(0.8/0.6) + base_mass(-1,3)kg + COM偏移±5cm + push_velocity±0.5m/s/10-15s + reset_pose(xy±0.5m, yaw±π)
140. **☲离·提取IsaacLab 观测结构** — 8项(lin_vel+ang_vel+gravity+cmd+joint_pos+joint_vel+action+height_scan), flat=48维, rough=235维(含187点高度扫描)
141. **☲离·提取IsaacLab Go1 rough地形缩放** — boxes_height(0.025,0.1) + random_rough_noise(0.01,0.06) + step=0.01 (因Go1体型小于ANYmal需缩小)
142. **☳震·go1_sim.py ISAAC_RL_CONFIG** — 完整整合: sim(dt/decimation/episode) + observation(8项dims/flat48/rough235) + rewards_rough(9项权重) + rewards_flat(2项覆盖) + domain_rand(friction/mass/push/reset) + terrain(Go1缩放) + termination + ppo_rough/flat超参 + wtw_control(Kp20/Kd0.5) + wtw_domain_rand
143. **☳震·go1_rl.py NVIDIA升级** — ISAAC_PPO_ROUGH/FLAT超参常量 + WTW_ACTUATOR_NET/WTW_RL_CHECKPOINT路径 + load_wtw_actuator_net()(成功加载16KB RecursiveScriptModule) + load_wtw_rl_policy()(11.7MB rsl_rl) + find_pretrained_models扩展(+walk-these-ways 2模型 + cpg-go1-sim 1模型)
144. **☵坎·全链路验证** — 9/9 py_compile PASS; go1_sim ISAAC_RL_CONFIG全字段正确; go1_rl PPO超参+WTW actuator net加载+49个预训练模型(28 quadruped-rl + 15 rl_sar + 3 GenLoco + 2 walk-these-ways + 1 cpg-go1-sim); go1_nvidia_integration 8维参数交叉验证正确

**Phase 22 伏羲八卦全功能E2E测试·7Bug修复·41/41 PASS (2026-03-10):**
145. **☲离·浏览器Dashboard验证** — 启动Brain API :8085, Dashboard加载: 🟢已连接 + 3D Go1渲染 + 57按钮 + 状态栏(0.264m/1.00/站立/98%) + WASD提示 + 6地形选择 + 5速度选择
146. **☳震·八卦E2E测试套件** — 创建_e2e_bagua_test.py(41项): ☰乾基础API(4) + ☷坤步态(7) + ☲离智能行为(6) + ☳震姿态(4) + ☴巽情感(5) + ☵坎语音(3) + ☶艮记忆健康(4) + ☱兑地形速度(5) + 演示(3)
147. **🔴 步态直接命令Bug** — `trot 2`/`pace 1`/`bound 1`返回"未知命令" → 需`gait trot 2`前缀。**修复**: command()添加17个步态直接动词别名(trot/pace/bound/gallop/crawl/pronk/spin/sidestep/backflip/wave/pushup/trot_run/climb/handshake/dance1/dance2/cpg)
148. **🔴 姿态命名子命令Bug** — `pose lean 0.3 1`导致`float("lean")`崩溃(connection reset)。**修复**: pose命令支持双模式解析: 命名式`pose lean 0.3 [dur]`和数值式`pose <l> <t> <l> <e> [dur]`, 加try/except防crash
149. **☵坎·修复后全链路验证** — **41/41 PASS · 0 FAIL**: 基础4✅ 步态7✅ 智能行为6✅ 姿态4✅ 情感5✅ 语音3✅ 记忆健康4✅ 地形速度5✅ 演示3✅

**Phase 23 后端物理引擎内核·架构突破·前后端分离 (2026-03-12):**
150. **☲离·全量深度审计** — 14/14 py_compile + 41/41 Brain E2E + 8/8依赖 + 30 refs + 54预训练模型全量验证通过
151. **☶艮·7个核心架构问题发现** — ①前端JS独立物理(Three.js内计算,无MuJoCo精度) ②CPG振荡器未从refs/cpg-go1-sim复用 ③IK仅前端JS无Python后端 ④奖励函数未从quadruped-rl-locomotion/IsaacLab复用 ⑤地形生成未从unitree-mujoco复用 ⑥WTW策略加载存在但未集成到仿真循环 ⑦WebSocket实时推送缺失(前端轮询HTTP)
152. **☳震·go1_engine.py创建** — 后端物理引擎内核(~550行), 6大子系统: §1 CPG振荡器(复用cpg-go1-sim Stein原理,7步态) §2 IK/FK求解器(复用gooddawg/open-quadruped) §3 奖励计算器(复用quadruped-rl-locomotion+IsaacLab) §4 地形管理器(复用unitree-mujoco TerrainGenerator) §5 MuJoCo物理引擎核心(状态提取+IMU+足端力+步态CPG→IK联动) §6 WebSocket服务器(RFC 6455纯stdlib实现,零外部依赖)
153. **☳震·go1_engine.py自测** — 29/29 PASS: CPG(7) + IK/FK(3) + 奖励(3) + 物理引擎(6) + 步态仿真(6) + 地形(4)
154. **☳震·go1_v8.html创建** — WebSocket驱动前端(纯可视化层): Three.js渲染+ACES色调映射+12关节实时驱动+足端接触力颜色+WASD键盘+步态/地形/速度侧边栏+RL奖励监视+关节角度面板+相机自动跟随
155. **☰乾·架构突破** — 从"前端独立物理"(v7)升级为"后端MuJoCo物理+前端纯渲染"(v8): WebSocket :8188实时推送50Hz物理状态→Three.js渲染, HTTP :8189提供API端点(/api/state /api/health /api/reward)
156. **☰乾·go1.py v4.0** — 统一CLI新增engine模块, 共9个模块(sim/brain/rl/control/test/resources/nvidia/hub/engine)
157. **☵坎·引擎HTTP API验证** — /api/health✅(sim_time+steps) /api/state✅(body_pos+quat+12关节+IMU+足端力+速度+力矩+动能) /api/reward✅(total+7项分解)
158. **☴巽·refs底层代码直接复用清单** — quadruped-rl-locomotion/go1_mujoco_env.py(奖励权重+终止条件+obs缩放) + unitree-mujoco/terrain_generator.py(AddStairs/AddRoughGround/AddPerlinHeighField) + cpg-go1-sim/config.py(5步态映射+Stein耦合矩阵) + walk-these-ways(actuator_net 16KB+RL策略12MB)
159. **☳震·RL深度整合到引擎** — _try_load_rl_resources()(WTW actuator net 16KB RecursiveScriptModule成功加载 + RL策略11.7MB识别) + _build_obs()(48维IsaacLab flat观测向量: lin_vel/ang_vel/projected_gravity/cmd/joint_pos_rel/joint_vel/last_action) + apply_domain_randomization()(WTW go1_config.py: friction[0.05,4.5] + mass[-1,3]kg)
160. **☵坎·增强自测36/36 PASS** — 新增§7 RL资源整合(5项: actuator net/RL策略/48维obs/obs有限) + §8 域随机化(2项: 质量/摩擦力)。修复numpy.bool_ identity检查bug
161. **☵坎·零回归验证** — Brain E2E 41/41 PASS(原有功能全部不受影响) + 11/11 py_compile PASS

**Phase 24 仿真核心深度整合·1804资源穷尽·功能矩阵覆盖 (2026-03-12):**
162. **☳震·go1_sim_core.py创建** — 仿真核心(~400行), 6大子系统全部从refs/底层代码直接移植: §1 SteinCPG(RK4积分+5步态神经元参数+数值稳定sigmoid) §2 Go1RewardSystem(12项奖励/惩罚+课程学习+healthy终止条件) §3 DomainRandomizer(9参数范围: friction/mass/COM/motor_strength等) §4 TerrainFactory(5种地形: flat/rough/stairs/slope/perlin+opensimplex) §5 AssetRegistry(本地资源扫描) §6 自测29/29 PASS
163. **☲离·本地资源穷尽扫描** — 发现**1804项可用资源**: 1243 STL + 174 URDF + 39 MuJoCo场景 + 54预训练模型 + 294 Mesh目录, 全部在refs/30个项目中
164. **☳震·go1_engine.py深度整合** — 接入sim_core双模式: cpg_stein(完整8神经元RK4) + cpg_simple(Bezier IK); reward_full(14维Go1RewardSystem) + reward_calc(7维快速); _use_stein标志切换; 36/36 PASS
165. **🔴 CPG Stein exp溢出修复** — stein_derivatives()中np.exp(b*(xi-p))的b=-2000导致溢出 → _safe_sigmoid()将参数裁剪到[-500,500], 零RuntimeWarning
166. **☵坎·全系统验证** — engine 36/36 + sim_core 29/29 + brain 41/41 + compile 12/12 = **106/106 PASS · 0 FAIL**
167. **☰乾·go1.py v4.0升级** — 新增core模块(go1_sim_core.py), 共10个模块: sim/brain/rl/control/test/resources/nvidia/hub/engine/core
168. **☴巽·CPG benchmark** — SteinCPG RK4: 5179 steps/s (10000步=1.93秒), 满足50Hz实时推送需求
