...
$env:ANDROID_SDK_ROOT = "090-构建与部署_Build\android-sdk"
& ".\gradlew.bat" assembleFDroidDebug --no-configuration-cache 2>&1 | Select-Object -Last 20
```
...
### Step 4: 安装
```powershell
& "D:\platform-tools\adb.exe" shell "cat /data/local/tmp/ss.apk | pm install -t -r -S <字节数>"
```
**成功标志**：`Success`
...
### Step 7: 日志检查（如果有异常）
```powershell
& "D:\platform-tools\adb.exe" logcat -d -t 30 --pid=$(& "D:\platform-tools\adb.exe" shell pidof info.dvkr.screenstream.dev) 2>$null
```
