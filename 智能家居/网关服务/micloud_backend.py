...
class MiCloudDirect:
    ...
    def quick_action(self, action: str) -> dict:
        """快捷操作: all_off, all_on, lights_off, lights_on, fans_off 等"""
        affected = []
        for d in self._devices:
            model = d.get("model", "")
            ...
            if action == "all_off" and domain in ("switch", "light", "fan"):
                ctrl_action = "turn_off"
            elif action == "all_on" and domain in ("switch", "light"):
                ctrl_action = "turn_on"
            elif action == "lights_off" and domain == "light":
                ctrl_action = "turn_off"
            elif action == "lights_on" and domain == "light":
                ctrl_action = "turn_on"
            elif action == "fans_off" and domain == "fan":
                ctrl_action = "turn_off"
            if ctrl_action:
                result = self.control_device(did, ctrl_action)
