...
评分算法：JPEG文件大小变异系数(CV=std/mean) 作为画面活跃度代理
"""
import json
import os
import subprocess
...
DEFAULT_DIR = os.environ.get('SCB_DIR', r"D:\屏幕录制\ai_segments")

QUALITY_PRESETS = {
    'low': {
        'scale': '1280:720', 'fps': 15, 'crf': 30, 'preset': 'ultrafast',
        'desc': '720p 15fps (最低CPU占用，适合文字/UI分析)',
    },
    'medium': {
        'scale': '1920:1080', 'fps': 24, 'crf': 26, 'preset': 'veryfast',
        'desc': '1080p 24fps (均衡)',
    },
    'high': {
        'scale': '2560:1440', 'fps': 30, 'crf': 23, 'preset': 'fast',
        'desc': '1440p 30fps (高质量)',
    },
    'source': {
        'scale': None, 'fps': 30, 'crf': 23, 'preset': 'fast',
        'desc': '原始分辨率 30fps',
    },
}
...
def is_file_locked(filepath: str) -> bool:
    ...
    # 方法2: 文件修改时间检测（FFmpeg不加锁但持续刷写，mtime会更新）
    try:
...
def score_segment(video_path: str, sample_interval: int = 2) -> float:
    ...
    if len(frames) >= 2:
        ...
        if mean_size > 0:
            ...
            # 综合: 70%变化量 + 30%绝对复杂度（解决全程高活动CV低的盲区）
            score = min(cv_score * 0.7 + complexity_score * 0.3, 1.0)
...
class ScreenBridge:
    ...
    def set_recording_prefix(self, prefix: Optional[str]):
        """设置当前录制文件前缀（由server在录制开始/停止时调用）"""
    ...
    def get_segments(self, since_minutes: Optional[int] = None,
        ...
        for f in files:
            ...
            if not locked and self._recording_prefix and str(f).startswith(self._recording_prefix):
                locked = True
