...
场景设计原则：
  - 每个场景调用3+种不同资源（感知/导航/系统/输入/Intent）
  - 每个场景包含条件分支（if/else决策）
  - 逐场景降低AI参与度：S1(L2+L1) → S2(L1) → S3(L1) → S4(L1) → S5(L0纯API)
...
def GET(p, **kw): return http("GET", p, **kw)
def POST(p, b=None, **kw): return http("POST", p, b, **kw)
...
status = GET("/status")
if "_error" in status:
    print(f"❌ API不可达: {BASE}/status"); sys.exit(1)
print(f"✓ API连接正常: {BASE}\n")
