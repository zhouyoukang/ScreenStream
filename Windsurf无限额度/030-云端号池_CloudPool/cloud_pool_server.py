...
RATE_LIMIT_ADMIN = 200  # admin endpoints higher limit
...
def audit_log(action, detail='', ip='', device_id=''):
    """Write audit trail for security-sensitive operations."""
    try:
        db = get_db()
        db.execute("INSERT INTO audit_log (action,detail,ip,device_id,created_at) VALUES (?,?,?,?,?)",
...
def _invalidate_pool_cache():
    """Invalidate public pool cache after data changes."""
    _pool_cache['ts'] = 0
...
def api_health():
    ...
    try:
        ...
        a = db.execute("SELECT COUNT(*) FROM accounts WHERE status='available' AND daily_pct>5").fetchone()[0]
...
def api_admin_overview():
    ...
    try:
        ...
        urgent = db.execute("SELECT COUNT(*) FROM accounts WHERE days_left<=3 AND days_left>0").fetchone()[0]
...
def api_public_pool(force=False):
    ...
    损之又损：使用内存缓存+ETag，10秒内同一数据不重查DB。"""
    now = time.time()
...
def api_ext_pull(device_id=''):
    ...
    Marks account as allocated to this device."""
    if not device_id:
...
def api_ext_push(data):
    """Push health data from extension to cloud."""
    accounts = data.get('accounts', [])
    device_id = data.get('device_id', '')
    if not accounts:
        return {'ok': False, 'error': 'no accounts'}
...
def api_ext_pull_blob(device_id='', email='', ip=''):
    """扩展直拉auth blob — 道之根本·无感换号之核心。
    Pull decrypted auth snapshot for hot-inject switching.
    If email specified, pull that account. Otherwise, pull best available.
    Returns auth blob that can be directly injected into state.vscdb."""
    if not device_id:
        return {'ok': False, 'error': 'device_id required'}
    db = get_db()
    try:
        acc = None
        if email:
            # Pull specific account
            acc = db.execute("SELECT * FROM accounts WHERE email=? AND auth_blob_enc!=''",
                            (email,)).fetchone()
        else:
            # Check if device already has an allocated account with blob
            acc = db.execute("""SELECT * FROM accounts WHERE device_id=? AND status='allocated'
                ...
                (device_id,)).fetchone()
            if not acc:
                # Find best available account with auth blob
                acc = db.execute("""SELECT * FROM accounts WHERE status='available'
                    AND daily_pct>5 AND weekly_pct>5 AND days_left>1
        ...
        return {
            ...
            'plan': acc['plan'],
            'daily': acc['daily_pct'],
            'weekly': acc['weekly_pct'],
            'days_left': acc['days_left'],
            'api_key_preview': acc['api_key_preview'],
            'auth_blob': auth_blob,  # The actual auth data for state.vscdb injection
            'harvested_at': acc['harvested_at'],
        }
    finally:
        db.close()
...
def api_ext_heartbeat(data, ip=''):
    """设备心跳 — 损之又损·最小数据交互。
    Device sends minimal health update, cloud returns if switch needed."""
    device_id = data.get('device_id', '')
    ...
    db = get_db()
    try:
        # Update current account health if provided
        if current_email and daily >= 0:
            ...
            db.commit()
            _invalidate_pool_cache()
        # Check if current account needs switching (quota exhausted)
        need_switch = False
        ...
        # Attach pending push directives to heartbeat response
...
def api_ext_release(data):
    """Release an account back to pool (when quota exhausted)."""
...
def api_device_info(device_id):
    ...
    try:
        ...
        db.execute("UPDATE devices SET last_seen=? WHERE id=?", (_now(), device_id))
        db.commit()
        return {'ok': True,
...
def api_p2p_init(data, ip=''):
    ...
    try:
        ...
        return {'ok': True, 'order_id': oid, 'amount_yuan': total_cents/100,
                'amount_display': f'¥{total_cents/100:.2f}', 'w_credits': w_amount,
...
def api_admin_p2p_confirm(data):
    """Admin manually confirms a P2P order and credits W to device."""
...
def api_admin_p2p_create(data, ip=''):
    """Admin creates a P2P order for a device (manual top-up)."""
    device_id = data.get('device_id', '')
    w_amount = int(data.get('w_credits', 100))
...
def _security_event(event_type, severity='info', ip='', device_id='', detail='', fingerprint=''):
    """Record security event for threat intelligence."""
    try:
        db = get_db()
...
def api_admin_push_create(data, ip=''):
    ...
    try:
        ...
        result = {'ok': True, 'directive_id': did, 'type': dtype, 'target': target,
...
def api_admin_push_list():
    """List all push directives (active + expired + revoked)."""
    db = get_db()
    try:
...
def api_admin_push_revoke(data):
    """Revoke a push directive immediately."""
...
def api_ext_get_directives(device_id='', client_version=''):
    """Client polls for active push directives targeting them.
    Returns only non-revoked, non-expired directives."""
    db = get_db()
    try:
        now = _now()
        ...
        for r in rows:
            ...
            try:
                ...
                pass
            directives.append(d)
        # Increment acked_count for returned directives
        for d in directives:
            db.execute("UPDATE push_directives SET acked_count=acked_count+1 WHERE id=?", (d['id'],))
        if directives:
            db.commit()
    ...
    def do_GET(self):
        ...
        try:
            if path in ('', '/public'):
                ...
                ip = self._get_client_ip()
                self._json(api_ext_pull_blob(device_id, email, ip))
...
def main():
    ...
    print(f'  Public:    / /api/public/pool (ETag) /api/public/start')
    print(f'  Extension: /api/ext/pull-blob /api/ext/heartbeat /api/ext/push')
    print(f'  Admin:     /api/admin/bulk-sync /api/admin/audit /api/admin/overview')
