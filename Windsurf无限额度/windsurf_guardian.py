...
def guardian_loop():
    """主守护循环"""
    log.info("=" * 50)
    log.info("Windsurf Guardian v1.0 started")
    log.info("=" * 50)
            ...
            if not check_port("127.0.0.1", 443):
                ...
                if PROXY_SCRIPT.exists() and CERT_PEM.exists():
                    ...
                    if check_port("127.0.0.1", 443):
                        log.info("✅ Self-built proxy started on :443")
                        state.pop("proxy_down_since", None)
                    else:
                        log.error("❌ Self-built proxy failed to start")
            else:
                state.pop("proxy_down_since", None)
