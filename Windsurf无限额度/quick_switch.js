...
async function fullChain(email, password) {
  ...
  for (let i = Math.max(0, b.length - 30); i < b.length - 2; i++) {
    if (b[i] === 0x30) { let v = 0, s = 0, p = i + 1; while (p < b.length) { const x = b[p++]; v |= (x & 0x7f) << s; if (!(x & 0x80)) break; s += 7; } if (v > 0 && v <= 10000) used = v / 100; }
    if (b[i] === 0x40) { let v = 0, s = 0, p = i + 1; while (p < b.length) { const x = b[p++]; v |= (x & 0x7f) << s; if (!(x & 0x80)) break; s += 7; } if (v > 0 && v <= 10000) total = v / 100; }
  }

  return { ok: true, email, token, credits: { used, total, remaining: Math.round(total - used) }, displayName: d.displayName || email.split("@")[0] };
}
  ...
  if (arg === "--all") {
    ...
    for (const a of accounts) {
      try {
        ...
        if (r.ok) {
          console.log(`  OK  ${a.email.split("@")[0].padEnd(30)} remaining: ${r.credits.remaining}  token: ${r.token.substring(0, 12)}...`);
          saveAccountStatus(a.email, r.credits.remaining > 10 ? "verified" : "low_credits", r.credits);
        } else {
