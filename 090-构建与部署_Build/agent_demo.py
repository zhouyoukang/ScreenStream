...
def act(endpoint, data=None, method="GET"):
    ...
    try:
        if method == "POST":
            r = requests.post(f"{BASE}{endpoint}", json=data, timeout=10)
        else:
            r = requests.get(f"{BASE}{endpoint}", timeout=10)
        ms = int((time.time()-t0)*1000)
        try:
            return {"ok": r.ok, "ms": ms, "data": r.json()}
        except Exception:
            return {"ok": r.ok, "ms": ms, "data": {}}
    except Exception as e:
        return {"ok": False, "ms": int((time.time()-t0)*1000), "data": {"error": str(e)}}
...
steps += 1
s1 = observe()
alipay_ok = "alipay" in s1["pkg"].lower()
if not alipay_ok and ("允许" in " ".join(s1["texts"]) or "打开" in " ".join(s1["clickables"])):
    steps += 1
    act("/findclick", {"text": "打开"}, "POST")
    wait_ms(3000)
    s1 = observe()
...
steps += 1
s2 = observe()
calc_ok = "calculator" in s2["pkg"].lower()
