...
### 笔记本 (MECHREVO WUJIE 14)

| 项 | 值 |
|----|---|
| **型号** | 机械革命 无界14 |
| **CPU** | AMD Ryzen 7 7840HS · 8C/16T · Zen4 4nm |
| **RAM** | 16GB LPDDR5X-6400 · **板载焊死不可升级** |
| **GPU** | AMD Radeon 780M · RDNA3 12CU |
| **SSD** | ①致钛1TB(E:) ②长鑫1TB(C:+D:) |
| **屏幕** | 2880×1800 · 120Hz · 200%缩放 |
| **电池** | 设计61.4Wh → 实际35.9Wh · **健康度58.6%** |
| **核心瓶颈** | 内存16GB焊死 · 电池58.6% · BIOS 2023-05未更新 |
| **已知通病** | 见 `诊断手册_笔记本硬件.md` §八 |
...
| Action | 用途 |
|--------|------|
| diagnose | 检查23项安全指标（含hosts/W3SVC/SSTP/LG/portproxy） |
| fix | 自动修复已知问题（14项，含hosts清理/W3SVC/SSTP） |
| protect | 部署agent_guard.json |
| hosts-guard | 持续监控hosts文件（60s间隔，自动清理windsurf/codeium条目） |
| report | 输出机器可读JSON诊断报告 |

### remote-agent/（远程诊断中枢）

WebSocket三体架构：浏览器（五感）+ PowerShell Agent（手）+ 分析引擎（脑）

```bash
cd 台式机保护/remote-agent && npm install && node server.js
# 浏览器打开 http://localhost:3002
# 目标电脑管理员PowerShell: irm http://<IP>:3002/agent.ps1 | iex
```

| 能力 | 实现 |
|------|------|
| 浏览器网络诊断 | DNS/HTTPS/IP 11项自动检测 |
| Agent 17步诊断 | hostname→firewall全链路扫描 |
| Clash/VPN识别 | 198.18.x fake-IP / DoH拦截自动判别 |
| hosts守护 | 60s自动清理windsurf/codeium条目 |
| 远程终端 | 浏览器执行PowerShell命令 |
| CLI工具 | `node brain.js exec/auto/state/say/msg` |
