...
_wx_cfg = _CFG.get("wechat", {})
if _wx_cfg.get("enabled"):
    import hashlib as _hl

    def test_wx_status():
        r = httpx.get(f"{GATEWAY_URL}/wx/status", timeout=5)
        d = r.json()
        return d.get("enabled") and d.get("router_ready"), f"enabled={d.get('enabled')}, router={d.get('router_ready')}"

    test("WeChat /wx/status", test_wx_status)

    def test_wx_verify():
        _ts = str(int(time.time()))
        _nc = "verify_test"
        _tk = _wx_cfg.get("token", "")
        _sig = _hl.sha1("".join(sorted([_tk, _ts, _nc])).encode()).hexdigest()
        r = httpx.get(f"{GATEWAY_URL}/wx", params={"signature": _sig, "timestamp": _ts, "nonce": _nc, "echostr": "OK"}, timeout=5)
        return r.text == "OK", f"echostr={'matched' if r.text == 'OK' else 'MISMATCH'}"

    test("WeChat GET /wx verify", test_wx_verify)
else:
    skip("WeChat /wx/status", "wechat not enabled in config")
    skip("WeChat GET /wx verify", "wechat not enabled in config")


# ============================================================
# 5. ScreenStream Smart Home Routes
# ============================================================
print("\n=== 5. ScreenStream Smart Home Routes ===")
