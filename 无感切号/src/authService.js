...
class AuthService {
  ...
  _vscdbRead(tmpName, pyExpr) {
    const dbPath = this._vscdbPath();
    if (!fs.existsSync(dbPath)) return null;
    const tmpDb = path.join(os.tmpdir(), tmpName);
    try { fs.copyFileSync(dbPath, tmpDb); } catch { return null; }
    try {
      const { execSync } = require('child_process');
      const pyCmd = pyExpr.replace(/__DB__/g, tmpDb.replace(/\\/g, '\\\\'));
      return execSync(pyCmd, { encoding: 'utf-8', timeout: 5000, maxBuffer: 200 * 1024 }).trim() || null;
    } finally {
  ...
  readCachedQuota() {
    try {
      ...
      try {
        ...
        raw = execSync(pyCmd, { timeout: 5000, encoding: 'utf8' }).trim();
      } finally {
        try { fs.unlinkSync(tmpDb); } catch {}
      }
      if (!raw) return null;
...
"`;
      raw = execSync(pyCmd, { timeout: 5000, encoding: 'utf8', maxBuffer: 200 * 1024 }).trim();
      } finally {
        try { fs.unlinkSync(tmpDb); } catch {}
      }
  ...
  readCachedValue(key) {
    try {
      return this._vscdbRead(`wam_read_${Date.now()}.db`, `python -c "import sqlite3,sys;conn=sqlite3.connect(r'__DB__');r=conn.cursor().execute('SELECT value FROM ItemTable WHERE key=?',('${key.replace(/'/g, "\\'")}',)).fetchone();print(r[0] if r else '');conn.close()"`);
    } catch { return null; }
  }
  ...
  readCurrentApiKey() {
    try {
      return this._vscdbRead('wam_apikey_read.vscdb', `python -c "import sqlite3,json;conn=sqlite3.connect(r'__DB__');r=conn.cursor().execute('SELECT value FROM ItemTable WHERE key=?',('windsurfAuthStatus',)).fetchone();conn.close();d=json.loads(r[0]) if r else {};print(d.get('apiKey',''))"`);
    } catch (e) {
