...
class AccountManager {
  ...
  _discoverExtensionAccounts() {
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (fs.existsSync(candidate) && path.normalize(candidate).toLowerCase() !== path.normalize(this._filePath).toLowerCase()) {
  ...
  _loadAndMergeAll() {
    ...
    // Source 4: Auto-discovered extension dirs (one-time migration only)
    const anySourceHasAccounts = primary.length > 0 || persistentSources.some(s => s.length > 0);
    const discovered = (this._isolated || anySourceHasAccounts) ? [] : this._discoverExtensionAccounts();
    ...
    // Merge remaining sources (using normalized email for comparison)
    let merged = 0;
    for (let s = 1; s < allSources.length; s++) {
      for (const ext of allSources[s]) {
        if (!ext.email) continue;
        const extKey = AccountManager.normalizeEmail(ext.email);
        const existing = this._accounts.findIndex(a => AccountManager.normalizeEmail(a.email) === extKey);
        if (existing < 0) {
    ...
    // 道法自然：无论数据从何而来，统一去重归一
    const dedupChanged = this._dedup();

    if (merged > 0 || dedupChanged) {
      console.log(`WAM: [MERGE] recovered ${merged} accounts from persistent storage! Total: ${this._accounts.length}`);
      this._save(); // Persist the merged/deduped result to all locations
    } else {
  ...
  _load() {
    // Legacy compat — just load primary file + dedup safety net
    this._accounts = this._loadFrom(this._filePath);
    if (this._dedup()) this._save();
  }
  ...
  /** 全局去重 — 按归一化email去重，保留最新数据（阴阳合一：多源归一） */
  _dedup() {
    const seen = new Map(); // normalizedEmail -> index in new array
    const deduped = [];
    for (const a of this._accounts) {
      const key = AccountManager.normalizeEmail(a.email);
      if (!key) continue;
      const existIdx = seen.get(key);
      if (existIdx === undefined) {
        seen.set(key, deduped.length);
        deduped.push({ ...a, email: key }); // normalize stored email
      } else {
        // Keep fresher data (merge fields from duplicate into existing)
        const existing = deduped[existIdx];
        const extChecked = a.usage?.lastChecked || a.lastChecked || 0;
        const locChecked = existing.usage?.lastChecked || existing.lastChecked || 0;
        if (extChecked > locChecked) {
          // Newer data wins, but preserve password if missing
          const pwd = existing.password || a.password;
          deduped[existIdx] = { ...existing, ...a, email: existing.email, password: pwd };
        } else {
          // Existing is fresher — just fill in missing password
          if (a.password && !existing.password) existing.password = a.password;
        }
      }
    }
    const removed = this._accounts.length - deduped.length;
    if (removed > 0) {
      console.log(`WAM: [DEDUP] removed ${removed} duplicates (${this._accounts.length} → ${deduped.length})`);
      this._accounts = deduped;
      return true;
    }
    return false;
  }
  ...
  findByEmail(email) {
    const key = AccountManager.normalizeEmail(email);
    const idx = this._accounts.findIndex(a => AccountManager.normalizeEmail(a.email) === key);
    return idx >= 0 ? { index: idx, account: { ...this._accounts[idx] } } : null;
  }

  add(email, password) {
    if (!email || !password || !email.includes('@')) return false;
    const normalized = AccountManager.normalizeEmail(email);
    if (this.findByEmail(normalized)) return false;
    this._accounts.push({ email: normalized, password, credits: undefined, loginCount: 0, addedAt: Date.now() });
    this._save();
    this._notify();
    return true;
  }
  ...
  addBatch(text) {
    ...
    const addedAccounts = [];
    for (const {email: rawEmail, password} of pairs) {
      const email = AccountManager.normalizeEmail(rawEmail);
      if (!email || this.findByEmail(email)) { skipped++; continue; }
  ...
  merge(externalAccounts) {
    ...
    for (const ext of externalAccounts) {
      ...
      const extKey = AccountManager.normalizeEmail(ext.email);
      const idx = this._accounts.findIndex(a => AccountManager.normalizeEmail(a.email) === extKey);
      if (idx < 0) {
        const newAccount = {
        ...
        if (ext.rateLimit && ext.rateLimit.until > Date.now()) {
          ...
          this._rateLimits.set(AccountManager.normalizeEmail(ext.email), ext.rateLimit);
        }
        ...
        if (ext.rateLimit && ext.rateLimit.until > Date.now() && !local.rateLimit) {
          ...
          this._rateLimits.set(AccountManager.normalizeEmail(local.email), ext.rateLimit);
          changed = true;
        }
  ...
  markRateLimited(index, resetsInSeconds = 3600, info = {}) {
    ...
    this._rateLimits.set(AccountManager.normalizeEmail(a.email), {
      until,
  ...
  isRateLimited(index) {
    const a = this.get(index);
    if (!a) return false;
    const rl = this._rateLimits.get(AccountManager.normalizeEmail(a.email));
    if (!rl) {
      // Check persisted state
      if (a.rateLimit && a.rateLimit.until > Date.now()) {
        // v6.8+ 提前恢复: message_rate=25% | tier_cap/capacity_matrix=50%
        const _rlType = a.rateLimit.type || '';
        const _isEarlyRecoverable = _rlType === 'message_rate' || _rlType === 'tier_cap' || _rlType === 'capacity_matrix';
        if (_isEarlyRecoverable) {
          const totalCooldown = (a.rateLimit.resetsIn || 1200) * 1000;
          const elapsed = totalCooldown - (a.rateLimit.until - Date.now());
          const recoveryRatio = _rlType === 'message_rate' ? 0.25 : 0.5;
          const minRecoveryMs = Math.max(60000, totalCooldown * recoveryRatio);
    ...
    if (rl.until <= Date.now()) {
      this._rateLimits.delete(AccountManager.normalizeEmail(a.email));
      if (index >= 0 && index < this._accounts.length) {
        delete this._accounts[index].rateLimit;
        this._save();
      }
    ...
    // v6.8+ 提前恢复: message_rate=25% | tier_cap/capacity_matrix=50%
    const _rlType2 = rl.type || '';
    const _isEarlyRecoverable2 = _rlType2 === 'message_rate' || _rlType2 === 'tier_cap' || _rlType2 === 'capacity_matrix';
    if (_isEarlyRecoverable2 && rl.hitAt) {
      const elapsed = Date.now() - rl.hitAt;
      const totalCooldown = (rl.resetsIn || 1200) * 1000;
      const recoveryRatio = _rlType2 === 'message_rate' ? 0.25 : 0.5;
      const minRecoveryMs = Math.max(60000, totalCooldown * recoveryRatio);
      if (elapsed >= minRecoveryMs) {
        const rem = this.effectiveRemaining(index);
        if (rem !== null && rem > 0) {
          ...
          console.log(`WAM: [RL] #${index+1} early recovery from ${_rlType2} limit (elapsed=${Math.round(elapsed/1000)}s/${Math.round(totalCooldown/1000)}s, remaining=${rem})`);
          return false;
        }
      }
    }
  ...
  getRateLimitInfo(index) {
    const a = this.get(index);
    if (!a) return null;
    const rl = this._rateLimits.get(AccountManager.normalizeEmail(a.email)) || a.rateLimit;
  ...
  markModelRateLimited(index, modelUid, resetsInSeconds = 600, info = {}) {
    ...
    const key = `${AccountManager.normalizeEmail(a.email)}|${modelUid}`;
  ...
  selectOptimal(excludeIndex = -1, threshold = 5, excludeIndices = []) {
    ...
    // Fallback: only truly unchecked accounts (skip recently-checked-but-null = likely depleted/failed)
    for (let i = 0; i < this._accounts.length; i++) {
      if (excluded.has(i)) continue;
      if (this.isRateLimited(i)) continue;
      if (this.isExpired(i)) continue;
      ...
      if (rem === null || rem === undefined) {
        const lastCheck = this._accounts[i].usage?.lastChecked;
        if (lastCheck && Date.now() - lastCheck < 1800000) continue; // 30min内查过但null → 跳过
        return { index: i, remaining: null, needsVerify: true };
      }
    }
    return null;
  }
