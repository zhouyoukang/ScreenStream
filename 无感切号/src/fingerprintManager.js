...
function _readStorageJson(storagePath) {
  try { return fs.existsSync(storagePath) ? JSON.parse(fs.readFileSync(storagePath, 'utf8')) : {}; } catch { return {}; }
}
function _writeStorageJson(storagePath, data) {
...
function resetFingerprint(options = {}) {
  ...
  try {
    ...
    // Update storage.json
    const storageData = _readStorageJson(paths.storageJson);
    for (const k of TELEMETRY_KEYS) storageData[k] = newIds[k];
    _writeStorageJson(paths.storageJson, storageData);
...
function restoreFingerprint(backupPath) {
  try {
    ...
    const storageData = _readStorageJson(paths.storageJson);
    for (const k of TELEMETRY_KEYS) { if (data.ids[k]) storageData[k] = data.ids[k]; }
    _writeStorageJson(paths.storageJson, storageData);
...
function ensureComplete() {
  ...
  try {
    ...
    for (const k of TELEMETRY_KEYS) {
      if (!storageData[k]) {
        storageData[k] = (k === 'telemetry.machineId' || k === 'telemetry.macMachineId') ? _hex32() : _uuid();
        fixed.push(k);
        changed = true;
      }
    }
    if (changed) _writeStorageJson(paths.storageJson, storageData);
  } catch (e) {
