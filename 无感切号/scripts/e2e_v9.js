...
test('version合法(semver)', () => /^\d+\.\d+\.\d+$/.test(pkg.version) ? true : pkg.version);
test('命令数=12', () => {
...
test('VSIX大小合理(30-2000KB)', () => {
  ...
  const latest = vsixFiles.map(f => ({f, sz: fs.statSync(path.join(BASE,f)).size})).sort((a,b)=>a.sz-b.sz)[0].f;
...
test('wisdom_bundle.json存在', () => fs.existsSync(path.join(BASE, 'data', 'wisdom_bundle.json')) ? true : '模板包不存在');
...
test('wisdom_bundle可解析', () => {
  try {
    const bundle = JSON.parse(fs.readFileSync(path.join(BASE, 'data', 'wisdom_bundle.json'), 'utf8'));
